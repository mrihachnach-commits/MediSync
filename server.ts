...
import { INITIAL_EVENTS, INITIAL_SYNC_STATUS } from './src/data/initialData.js';
...
const dongBoCalendarDeclaration: FunctionDeclaration = {
  name: 'dong_bo_calendar',
  description: 'Đồng bộ 2 chiều dữ liệu lịch làm việc với Google Calendar và Microsoft Outlook Calendar.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      targetPlatform: {
        type: Type.STRING,
        description: 'Nền tảng đồng bộ: google, outlook, hoăc both (cả hai)',
      },
      forceSync: {
        type: Type.BOOLEAN,
        description: 'Ép buộc đồng bộ tức thì tất cả sự kiện',
      },
    },
...
app.post('/api/calendar/sync', (req, res) => {
  ...
  if (target === 'google' || target === 'both') {
    syncStatus.googleConnected = true;
    syncStatus.googleLastSync = nowStr;
  }
  if (target === 'outlook' || target === 'both') {
    syncStatus.outlookConnected = true;
    syncStatus.outlookLastSync = nowStr;
  }

  scheduleEvents = scheduleEvents.map((evt) => ({
    ...evt,
    syncedGoogle: target === 'google' || target === 'both' ? true : evt.syncedGoogle,
    syncedOutlook: target === 'outlook' || target === 'both' ? true : evt.syncedOutlook,
  }));

  syncStatus.totalSyncedEvents = scheduleEvents.length;

  res.json({
    success: true,
    message: `Đã đồng bộ thành công ${scheduleEvents.length} lịch hẹn với ${target.toUpperCase()} Calendar!`,
    syncStatus,
  });
});
...
app.get('/api/schema', (req, res) => {
  res.json({
    ...
    architectureSteps: [
      ...
      {
        id: '5',
        title: 'Đồng bộ 2 chiều Google & Outlook Calendar',
        desc: 'Tự động tạo/sửa/xóa sự kiện, gán color-coding và xuất chuẩn iCal (.ics) cho Google / Microsoft Graph API.',
...
[QUY TẮC]: Tự cập nhật thói quen mới bằng hàm \`ghi_nho_thoi_quen\`.`;
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      if (name === 'tao_lich_hen') {
        ...
        scheduleEvents.push(newEvt);
        executedCall = { name, args, result: { success: true, createdId: newEvt.id } };
      ...
      } else if (name === 'dong_bo_calendar') {
        const platform = args.targetPlatform || 'both';
        syncStatus.googleLastSync = new Date().toLocaleString('vi-VN');
        syncStatus.outlookLastSync = new Date().toLocaleString('vi-VN');
        syncStatus.totalSyncedEvents = scheduleEvents.length;

        scheduleEvents = scheduleEvents.map((e) => ({ ...e, syncedGoogle: true, syncedOutlook: true }));

        replyText =
          replyText ||
          `🔄 Em đã thực hiện đồng bộ 2 chiều thành công toàn bộ ${scheduleEvents.length} lịch hẹn lên Google Calendar và Microsoft Outlook Calendar!`;
        executedCall = { name, args, result: { success: true, syncedCount: scheduleEvents.length } };
