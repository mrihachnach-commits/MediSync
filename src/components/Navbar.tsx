...
import { Calendar, LayoutGrid, ShieldCheck, RefreshCw, Mic, Cpu, Sparkles, Stethoscope, Clock, Sliders } from 'lucide-react';
import { CalendarSyncStatus, AppSettings } from '../types';

interface NavbarProps {
  activeTab: 'calendar' | 'matrix' | 'analytics' | 'sync' | 'architecture';
  setActiveTab: (tab: 'calendar' | 'matrix' | 'analytics' | 'sync' | 'architecture') => void;
  ...
  return (
    <header className="bg-[#0F172A] border-b border-slate-800 text-white sticky top-0 z-30 shadow-lg">
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="font-bold text-base md:text-lg text-slate-100 tracking-tight">
                {settings.doctorTitle || 'BS. Chẩn đoán Hình ảnh'}
              </h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="bg-emerald-950/80 border border-emerald-800/80 text-emerald-300 text-xs px-2.5 py-1.5 rounded-xl flex items-center gap-1.5 font-medium shadow-sm">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            <span>Bảo vệ Tối Nghỉ Ngơi: 100%</span>
          </div>

          <div className="bg-slate-800/80 border border-slate-700/80 text-slate-300 text-xs px-2.5 py-1.5 rounded-xl flex items-center gap-1.5 font-medium shadow-sm">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <span>Google & Outlook Sync: Ok</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 flex items-center gap-1 overflow-x-auto border-t border-slate-800/80 scrollbar-none">
        ...
        >
          ...
          <span>Đồng Bộ Google & Outlook</span>
        </button>
